...
The following `R` packages will be used throughout:

```{r package_list, eval = FALSE}
...
one can compare the "true" results to those from the complete, missing/observed and imputed data.
...
several post-hoc analyses using the data in all three states of completion.
...
``` {r ampute_freq, fig.width=350/72, fig.height=350/72, fig.align='center'}
...
```{r ampute_weights, fig.width=350/72, fig.height=350/72, fig.align='center'}
...
In addition to the `md.pattern` plot we can look at box plots available in the `mice`
package to insure that the data distributions by pattern meet our expectations.

``` {r ampute_boxplots, fig.width=500/72, fig.height=350/72, fig.align='center'}
bwplot(test.result, which.pat = 2, descriptives = TRUE)
...
```{r vim1, fig.width=350/72, fig.height=350/72, fig.align='center'}
...
We also produce default arguments for the `mice::mice` function for the `method` and 
`predictorMatrix` arguments.
...
###  Step 1 - Create `M` imputed data sets
...
data. We also identical analyses using both the complete and observed data. We proceed
...
longitudinal student data for students nested within schools. It also provides us with
...
``` {r pmm_comptable, include = FALSE, results="asis"}
options(datatable.print.colnames = "none")
knitr::knit_print(comp_table, class = F)
options(datatable.print.colnames = "auto")
```
...
``` {r pmm_mss_cat, fig.width=500/72, fig.height=400/72, fig.align='center'}
mss.cat
```

``` {r pmm_mss_pctm, fig.width=500/72, fig.height=400/72, fig.align='center'}
mss.pctm
```

``` {r pmm_msgp_cat, fig.width=500/72, fig.height=400/72, fig.align='center'}
msgp.cat
```

``` {r pmm_msgp_pctm, fig.width=500/72, fig.height=400/72, fig.align='center'}
msgp.pctm
```
